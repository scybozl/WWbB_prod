...
class Hw7ConfigLHEF(hw7Config.Hw7Config):
  ...
  def __init__(self, genSeq, runArgs, run_name="Herwig", beams="pp"):
    ...
    self.set_lhef_sherpa_commands = False
...
""".format(self.run_name)
  ...
  def run(self):
    ...
    if not self.set_lhef_mg5amc_commands and not self.set_lhef_powhegbox_commands and not self.set_lhef_sherpa_commands:
      raise RuntimeError(hw7Utils.ansi_format_error("Please use either 'lhef_mg5amc_commands()' or 'lhef_powhegbox_commands()' or 'lhef_sherpa_commands()' in the JobOptions!"))
...
"""
  ...
  def lhef_sherpa_commands(self, lhe_filename="events.lhe", me_pdf_order="NLO"):

    if not me_pdf_order in ["LO", "NLO"]:
      raise RuntimeError(hw7Utils.ansi_format_error("Herwig7ConfigLHEF.py:lhef_sherpa_commands: Parameter 'me_pdf_order' must either be 'LO' or 'NLO'!"))

    self.set_lhef_sherpa_commands = True

    self.__lhef_commands(lhe_filename, me_pdf_order)
